# Development Environment for the Runny

Docker running Nginx, PHP-FPM, Composer, Postgresql.
...
* [ExpressGateway](https://hub.docker.com/_/express-gateway)
...
```sh
git clone https://gitlab.com/CattoSistemas/runnyapi.git
```
...
For a better integration of Docker to PHPStorm, use the [documentation](https://gitlab.com/CattoSistemas/runnyapi/blob/master/doc/phpstorm-macosx.md).
...
3. Open your favorite browser :
    ...
    * [https://localhost:3000](https://localhost:3000/) Web ([HTTPS](#configure-nginx-with-ssl-certificates) not configured by default)
...
| postgres-dump    | Create backup of all databases               |
| postgres-restore | Restore backup of all databases              |
...
```sh
pgsql -U "$POSTGRES_USER" -h localhost -p "$POSTGRES_PASSWORD"
```

#### Creating backup the database

```sh
source .env && sudo docker exec $(sudo docker-compose ps -q postgresql) pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DATABASE" > "data/db/dumps/db.pgbkp"
```

#### Restore backup the database

```sh
source .env && docker exec $(sudo docker-compose ps -q postgresql) pg_restore -U "$POSTGRES_USER" -d "$POSTGRES_DATABASE"  "data/db/dumps/db.pgbkp"
```